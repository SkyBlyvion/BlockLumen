...
CREATE TABLE `Wallet_Holding` (
    ...
    CONSTRAINT `fk_wh_wallet` FOREIGN KEY (`wallet_id`) REFERENCES `Wallet` (`wallet_id`) ON DELETE CASCADE
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4;
...
CREATE TABLE `Trade` (
    ...
    -- FK vers Wallet_Holding(holding_id)
    `crypto_symbol` VARCHAR(10) NOT NULL,
    ...
    CONSTRAINT `fk_trade_holding` FOREIGN KEY (`holding_id`) REFERENCES `Wallet_Holding` (`holding_id`) ON DELETE CASCADE
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4;
...
CREATE TABLE `User_Learn` (
    `user_learn_id` INT NOT NULL AUTO_INCREMENT,
    `user_id` INT NOT NULL,
    -- FK vers User(user_id)
    `learn_id` INT NOT NULL,
    -- FK vers Learn(learn_id)
    `is_completed` BOOLEAN NOT NULL DEFAULT FALSE,
    `completed_at` DATETIME NULL,
    PRIMARY KEY (`user_learn_id`),
    UNIQUE KEY `uniq_ul_user_learn` (`user_id`, `learn_id`),
    ...
    CONSTRAINT `fk_ul_learn` FOREIGN KEY (`learn_id`) REFERENCES `Learn` (`learn_id`) ON DELETE CASCADE
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4;